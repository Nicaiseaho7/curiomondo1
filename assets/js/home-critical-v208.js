...
window.cmLoadFullRuntime=function(){
  ...
  runtimePromise=loadScript('assets/js/curiomondo-discovery-v208.js?v=208')
    .then(function(){return loadScript('assets/js/home-original-v101.js?v=208');})
    .then(function(){document.removeEventListener('input',inputHandler);document.removeEventListener('click',clickHandler);document.removeEventListener('keydown',keyHandler);return loadScript('assets/js/curiomondo-orbit-turbo-v34.js?v=208');})
...
if('IntersectionObserver'in window){var targets=[document.getElementById('cards'),document.getElementById('usefulHub')].filter(Boolean);var rio=new IntersectionObserver(function(es){if(es.some(function(x){return x.isIntersecting;})){rio.disconnect();requestRuntime();}},{rootMargin:'700px 0px'});targets.forEach(function(t){rio.observe(t);});}
window.addEventListener('pointerdown',requestRuntime,{once:true,passive:true});
