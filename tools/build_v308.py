...
def book_page():
 ...
 for i,(heading,paras) in enumerate(BOOK_PAGES,1):
  ...
  pages.append(f'<section{attrs}><span class="cm-book-kicker">eBook CurioMondo · {DATE_LABEL}</span><h1>{escape(BOOK_TITLE)}</h1>{sub}{body}<span class="cm-book-page-number">{i} / {len(BOOK_PAGES)}</span></section>')
