...
def _related(article: dict[str, Any]) -> list[dict[str, str]]:
    ...
    current_url = f"/notizie/{article.get('slug', '')}.html"
    category = str(article.get("categoria", "")).casefold()
    source = " ".join((str(article.get("titolo", "")), str(article.get("sommario", "")), category)).casefold()
    stop = {"della", "delle", "degli", "dello", "alla", "alle", "agli", "nella", "nelle",
            "negli", "oggi", "dopo", "come", "sono", "anche", "notizia", "notizie", "italia",
            "mondo", "settembre", "2026"}
    terms = {w for w in re.findall(r"[a-zà-öø-ÿ0-9]+", source) if len(w) > 4 and w not in stop}
    ranked = []
    for item in items:
        if item.get("url") == current_url or not str(item.get("url", "")).startswith("/notizie/"):
            continue
        haystack = " ".join((str(item.get("title", "")), str(item.get("excerpt", "")),
                             str(item.get("section", "")))).casefold()
        overlap = sum(2 if len(term) > 7 else 1 for term in terms if term in haystack)
        same_category = bool(category and category in str(item.get("section", "")).casefold())
        score = overlap + (5 if same_category else 0)
        if score >= 4:
            ranked.append((score, item))
    ranked.sort(key=lambda pair: (pair[0], str(pair[1].get("dateISO", ""))), reverse=True)
    return [{"url": item["url"], "title": item["title"]} for _, item in ranked[:3]]


# I tre pulsanti sotto la firma fanno parte dell'articolo CurioMondo come il
# titolo e le fonti: senza, la pagina prodotta qui sarebbe piu povera di una
# scritta a mano. Il comportamento arriva da curiomondo-article-v210.js, che
# questa pagina carica gia in fondo.
AZIONI = (
    '<div class="actions">'
